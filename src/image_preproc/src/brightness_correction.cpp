...
void BrightnessCorrection::correctGamma(const cv::InputArray& src, cv::OutputArray& dst, const double gamma) {
...
double BrightnessCorrection::computeOptimalGamma(const cv::InputArray& src) {
  // check image
  // TODO
...
void BrightnessCorrection::stretchHist(const cv::InputArray& src, cv::OutputArray& dst, const double percentage) {
  //std::cout << "Histo: " << percentage << std::endl;
  // check image
  // TODO
  cv::Mat img = src.getMat().clone();
...
void BrightnessCorrection::computeOptimalHist(const cv::InputArray& src, cv::OutputArray& dst) {
  cv::equalizeHist(src.getMat(), dst.getMat());
}
