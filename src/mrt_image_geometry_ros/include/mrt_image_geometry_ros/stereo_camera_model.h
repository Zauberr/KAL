...
inline const PinholeCameraModel& StereoCameraModel::left() const {
    return left_;
}
inline const PinholeCameraModel& StereoCameraModel::right() const {
    return right_;
}

inline std::string StereoCameraModel::tfFrame() const {
    return left_.tfFrame();
}

inline const cv::Matx44d& StereoCameraModel::reprojectionMatrix() const {
    return Q_;
}
