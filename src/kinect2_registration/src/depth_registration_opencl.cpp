...
#define PRINT_CL_ERROR(expr, err)                                                                 \
    OUT_ERROR(FG_BLUE "[" << CL_FILENAME << "]" FG_CYAN "(" << __LINE__ << ") " FG_YELLOW << expr \
                          << FG_RED " failed: " << err)
...
DepthRegistrationOpenCL::DepthRegistrationOpenCL() : DepthRegistration() {
    data = new OCLData;
}
...
bool DepthRegistrationOpenCL::init(const int deviceId) {
    ...
    CHECK_CL_ON_FAIL(
        data->program.build(options.c_str()), OUT_ERROR("failed to build program: " << err);
    ...
    CHECK_CL_PARAM(data->bufferRegistered =
                       cl::Buffer(data->context, CL_MEM_READ_WRITE, data->sizeRegistered, NULL, &err));
    ...
    CHECK_CL_PARAM(data->bufferOutput = cl::Buffer(
                       data->context, CL_MEM_WRITE_ONLY | CL_MEM_ALLOC_HOST_PTR, data->sizeRegistered, NULL, &err));
    ...
    CHECK_CL_PARAM(data->dataOutput = (unsigned char*)data->queue.enqueueMapBuffer(
                       data->bufferOutput, CL_TRUE, CL_MAP_READ, 0, data->sizeRegistered, NULL, NULL, &err));
    return true;
}
...
bool DepthRegistrationOpenCL::registerDepth(const cv::Mat& depth, cv::Mat& registered) {
    ...
    CHECK_CL_RETURN(data->queue.enqueueWriteBuffer(
        data->bufferDepth, CL_FALSE, 0, data->sizeDepth, depth.data, NULL, &eventZero[0]));
    CHECK_CL_RETURN(data->queue.enqueueNDRangeKernel(
        data->kernelSetZero, cl::NullRange, range, cl::NullRange, NULL, &eventZero[1]));

    CHECK_CL_RETURN(data->queue.enqueueNDRangeKernel(
        data->kernelRemap, cl::NullRange, range, cl::NullRange, &eventZero, &eventRemap[0]));

    CHECK_CL_RETURN(data->queue.enqueueNDRangeKernel(
        data->kernelProject, cl::NullRange, range, cl::NullRange, &eventRemap, &eventProject[0]));

    CHECK_CL_RETURN(data->queue.enqueueNDRangeKernel(
        data->kernelCheckDepth, cl::NullRange, range, cl::NullRange, &eventProject, &eventCheckDepth1[0]));

    CHECK_CL_RETURN(data->queue.enqueueNDRangeKernel(
        data->kernelCheckDepth, cl::NullRange, range, cl::NullRange, &eventCheckDepth1, &eventCheckDepth2[0]));

    CHECK_CL_RETURN(data->queue.enqueueReadBuffer(
        data->bufferRegistered, CL_FALSE, 0, data->sizeRegistered, data->dataOutput, &eventCheckDepth2, &eventRead));
    ...
    data->timings[0] += eventZero[0].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventZero[0].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[1] += eventZero[1].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventZero[1].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[2] += eventRemap[0].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventRemap[0].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[3] += eventProject[0].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventProject[0].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[4] += eventCheckDepth1[0].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventCheckDepth1[0].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[5] += eventCheckDepth2[0].getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventCheckDepth2[0].getProfilingInfo<CL_PROFILING_COMMAND_START>();
    data->timings[6] += eventRead.getProfilingInfo<CL_PROFILING_COMMAND_END>() -
                        eventRead.getProfilingInfo<CL_PROFILING_COMMAND_START>();

    if (++data->count == 100) {
        double sum = data->timings[0] + data->timings[1] + data->timings[2] + data->timings[3] + data->timings[4] +
                     data->timings[5] + data->timings[6];
