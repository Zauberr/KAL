...
namespace obstacle_detection_ros_tool {
    ...
    ObstacleDetection::ObstacleDetection(ros::NodeHandle nhPublic, ros::NodeHandle nhPrivate)
        ...
        //for grid_pub_temp
/*
        grid_pub_temp.data.assign((info.width * info.height), 0);

        grid_pub_temp.header.frame_id = "world";

        grid_pub_temp.info = info;
...
*/
    ...
    void ObstacleDetection::process() {
        ...
        if(transform_set_) {
            ...
            seg.setMaxIterations(150);  //bis hier 19 07 07
    ...
    void ObstacleDetection::mapImport(const pcl::PointCloud<pcl::PointXYZ>::Ptr &input, nav_msgs::OccupancyGrid& output) {
        ...
        for (int i = 0; i < input->size(); ++i) {
            ...
            //std::cout<<"point x= "<<x_pos_in<<" pointy= "<<y_pos_in<<std::endl;
    ...
    /*
     void ObstacleDetection::sum(std::vector<int8_t> grid){
        //static int output[info.width*info.height];
        for(int i = 0 ; i <=info.width*info.height-1; i++){
            grid_pub.data[i]=grid[i]+grid_pub.data[i];
        }
